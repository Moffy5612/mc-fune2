...
const MenuComponent = ({id, imgsrc, title, connection, isMobile}:{id:number, imgsrc:string, title:string, connection:boolean, isMobile:boolean}) => {
    
    const context = useContext(PageContext)

    return(
        <Grid item className="menu-content" onClick={()=>{if(connection)context?.setPage(id)}} xs={isMobile ? 16 : 4}>
